...
CoreOUT Core::stage2(CoreOUT sCO12, Instruction *stage12, char currentChar) {
    ...
    case RANGE:
        if (char(stage12->getChar1())<=currentChar && (char(stage12->getChar2())>=currentChar)) {
            valid = true;
            newPC = CoreOUT(sCO12.getPC()+1, sCO12.getCC_ID()+1);
            if (verbose) { printf("\t\tRANGE: Characters in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
        } else {
            if (verbose) { printf("\t\tRANGE: Characters not in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
            newPC = CoreOUT();
        }
        break;
    ...
    case NOT_RANGE:
        if (char(stage12->getChar1())> currentChar || (char(stage12->getChar2())< currentChar)) {
            valid = true;
            newPC = CoreOUT(sCO12.getPC()+1, sCO12.getCC_ID());
            if (verbose) { printf("\t\tNOT RANGE: Characters not in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
        } else {
            if (verbose) { printf("\t\tNOT RANGE: Characters in range: input %c from %c to %c\n", currentChar, char(stage12->getChar1()), char(stage12->getChar2()));}
            newPC = CoreOUT();
        }
        break;
