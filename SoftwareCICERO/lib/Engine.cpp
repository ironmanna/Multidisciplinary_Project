...
std::tuple<bool, int> Engine::runMultiChar(const std::string& _input) {
    reset(_input);
    ...
    while (core->isRunning()) {
        ...
        case CONTINUE:
            break;
        case ACCEPTED:
            return std::make_tuple(true, currentClockCycle);
        case REFUSED:
            return std::make_tuple(false, currentClockCycle);
        }
    }
