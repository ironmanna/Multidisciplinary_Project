...
class Re2CoproInstr:
	...
	def __init__(self, pc, instr_type, data1, data2 = None):
		self.pc   = pc
		self.type = instr_type
		if data2 == None:
			#self.data = data1 << 8
...
class MatchRange(Re2CoproInstr):
	def __init__(self,pc, char1, char2):
		super().__init__(pc, Re2CoproInstr_type.MATCH_RANGE, char1, char2)

	def dotty_str(self):
		return (f"{self.pc} -> {self.pc+1}\n"+
				f"{self.pc} [label =\"{self.pc} : {self.data}\" color=\"black\" fillcolor=\"#ffa822\" style=\"filled\"]\n"             )

class NotMatchRange(Re2CoproInstr):
	def __init__(self,pc, char1, char2):
		super().__init__(pc, Re2CoproInstr_type.NOT_MATCH_RANGE, char1, char2)
