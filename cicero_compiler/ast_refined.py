...
class match_character_range(ast_refined_node):
	def __init__(self, character1, character2):
		super().__init__()

		if isinstance(character1, str):
			self.character1 = bytes(character1, 'utf-8')[0]
		else:
			self.character1 = character1
		if isinstance(character2, str):
			self.character2 = bytes(character2, 'utf-8')[0]
		else:
			self.character2 = character2

	def dotty_str(self):
		# printable Ascii \x20-\x7F
		char1 = chr(self.character1)
		if self.character1 == 32:
			char1 = "SPACE"
		elif self.character1 < 32 or self.character1 > 127 or char1 in "\"\\":
			char1 = hex(self.character1)

		char2 = chr(self.character2)
		if self.character2 == 32:
			char2 = "SPACE"
		elif self.character2 < 32 or self.character2 > 127 or char2 in "\"\\":
			char2 = hex(self.character2)
		return f" {id(self)} [label=\"{char1}\"] [label=\"{char2}\"]"
