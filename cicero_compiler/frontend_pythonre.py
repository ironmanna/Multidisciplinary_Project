...
def match_negative_group_range(char_range_list):
	char_range_list.sort(key= lambda x : x[0])
	char_range_list = merge_ranges(char_range_list)
	list_of_ranges = [ast_refined.not_match_character_range(cl[0], cl[1]) for cl in char_range_list]
	return ast_refined.none_of(*list_of_ranges)

def match_positive_group_range(char_range_list):
	char_range_list.sort(key= lambda x : x[0])
	char_range_list = merge_ranges(char_range_list)
	list_of_ranges = [ast_refined.match_character_range(cl[0], cl[1]) for cl in char_range_list]
	return ast_refined.alternative(*list_of_ranges)
...
def p_group(p):
	...
	if  len(p) >= 4:
		...
		# Produce a list of two chars, first lower
		# NOTE: char is described by means of an int
		p[0] = [chars_range(p[1], p[3])]
